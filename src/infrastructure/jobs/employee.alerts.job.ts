...
async function processEmployee(
  ...
  for (const license of licenses) {
    ...
    if (days <= alertDays) {
      const licId = (license as unknown as { _id: ObjectId })._id?.toHexString() ?? 'unknown';
      candidates.push({
        ...
        documentType:  license.type === 'federal' ? 'federal_license' : 'state_license',
  ...
  for (const doc of documents) {
    ...
    if (days <= alertDays) {
      const docId = (doc as unknown as { _id: ObjectId })._id?.toHexString() ?? 'unknown';
      candidates.push({
      ...
      const { isDuplicate } = await submitTask(
        {
          ...
          area:        'hr',
          assignedTo:  profile.managerId ? profile.managerId.toHexString() : null,
          assignedBy:  SYSTEM_ACTOR_ID,
          participants: [userId],
          status:      'open',
          entity:      'Employee',
          entityId:    userId,
