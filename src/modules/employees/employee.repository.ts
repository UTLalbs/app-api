...
import { encryptAccountNumber, encryptClabe, getLastFour } from './employee.encryption';
import type {
  AuditLogEntry,
  BankAccount,
  ChecklistItem,
...
export async function findAllEmployees(
  ...
  if (filter.employmentStatus) query['employeeProfile.employmentStatus'] = filter.employmentStatus;
...
export async function addEmergencyContact(
  ...
  const result = await getUserCollection().findOneAndUpdate(
    ...
    { $push: { 'employeeProfile.emergencyContacts': newContact }, $set: { updatedAt: new Date() } },
    { returnDocument: 'after', projection: EMPLOYEE_PROJECTION },
  );
...
export async function removeEmergencyContact(
  ...
  const result = await getUserCollection().updateOne(
    ...
    {
      $pull: { 'employeeProfile.emergencyContacts': { _id: new ObjectId(contactId) } },
...
export async function updateBankAccount(
  ...
  if (fields.bankName    !== undefined) setFields['employeeProfile.bankAccounts.$.bankName']    = fields.bankName;
  if (fields.documentUrl !== undefined) setFields['employeeProfile.bankAccounts.$.documentUrl'] = fields.documentUrl;
  if (fields.isDefault   !== undefined) setFields['employeeProfile.bankAccounts.$.isDefault']   = fields.isDefault;
...
export async function removeEmployeeDocument(
  ...
  const user = await getUserCollection().findOne(
    { _id: new ObjectId(id), 'employeeProfile.documents._id': new ObjectId(docId) },
