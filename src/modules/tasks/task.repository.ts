...
export async function createTask(dto: CreateTaskDto): Promise<Task> {
  ...
  return toTask(
    { _id: result.insertedId, ...doc } as TaskDocument,
    { populateAssignedTo: true, populateParticipants: true },
  );
}
...
export async function findAllTasks(
  ...
  const tasks = await Promise.all(
    docs.map((doc) =>
      toTask(doc as TaskDocument, { populateAssignedTo: true }),
    ),
  );
