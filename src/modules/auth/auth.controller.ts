...
import { loginWithOIDC, refreshSession, logout, logoutAllDevices } from './auth.service';
import {
...
export const googleCallback = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (!pkce) throw new AuthError('Invalid or expired state — please try again');
    ...
    const profile = await handleGoogleCallback(currentUrl, state, pkce.codeVerifier);

    // Obtener orgId — en producción viene del subdominio o de la sesión
    // TODO Phase 4: resolver orgId desde subdominio o parámetro de registro
    const orgId = req.query.orgId as string ?? req.cookies?.pending_org_id;
    if (!orgId) throw new AuthError('Organization not specified');
    ...
    res.redirect(`${process.env.FRONTEND_URL ?? 'http://localhost:5173'}${redirectUrl}`);
  },
);
